...
const DRILLS: Drill[] = [
  ...
  makeDrill({ id: 'w_build', category: 'wedges', mode: 'build', requires: ['short_game', 'range'], primarySkill: 'distance_control', minutes: 12, minMinutes: 8 }),
  makeDrill({ id: 'w_press', category: 'wedges', mode: 'pressure', requires: ['short_game', 'range'], primarySkill: 'distance_control', minutes: 12, minMinutes: 8 }),

  makeDrill({ id: 'c_build', category: 'chipping', mode: 'build', requires: ['short_game'], primarySkill: 'strike', minutes: 10, minMinutes: 6 }),
  makeDrill({ id: 'd_build', category: 'driving', mode: 'build', requires: ['range'], primarySkill: 'start_line', minutes: 12, minMinutes: 8 }),
  ...
  makeDrill({ id: 'h_build', category: 'chipping', mode: 'build', requires: ['home'], primarySkill: 'strike', minutes: 10, minMinutes: 6 }),
...
const scored = (blocks: { drillId: string }[]) =>
...
describe('generateSession', () => {
  ...
  it('never includes a course (Ghost Nine) drill, even at a course venue', () => {
    const { blocks } = generateSession({
      minutes: 60,
      capabilities: ['course', 'putting_green', 'range', 'short_game'],
      allDrills: DRILLS,
    })
    for (const b of scored(blocks)) {
